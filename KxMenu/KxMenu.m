...
- (id)init
{
    ...
    if(self) {
        ...
        self.layer.shadowRadius = [KxMenu sharedMenu].cornerRadius;
    }
             ...
             menuItems:(NSArray *)menuItems
{
    ...
    if ([KxMenu sharedMenu].defineWidth>0) {
        _contentView.width =[KxMenu sharedMenu].defineWidth;
        _contentView.layer.masksToBounds = YES;
    }
    if ([KxMenu sharedMenu].defineHeight>0) {
        _contentView.height =[KxMenu sharedMenu].defineHeight;
    }
...
- (void)dismissMenu:(BOOL) animated
{
    ...
    if ([KxMenu sharedMenu].dismissBlock) {
        [KxMenu sharedMenu].dismissBlock(nil);
    }
}
...
- (UIView *) mkContentView
{
    ...
    if ([KxMenu sharedMenu].defineHeight >0 &&_menuItems.count>0) {
        maxItemHeight = ([KxMenu sharedMenu].defineHeight - 8) /_menuItems.count;
    }
...
//        if ([KxMenu sharedMenu].defineWidth>0) {
//            itemView.width =[KxMenu sharedMenu].defineWidth;
//        }
//        if ([KxMenu sharedMenu].defineHeight>0) {
//            itemView.height =[KxMenu sharedMenu].defineHeight;
//        }
        ...
        if (menuItem.title.length) {
            ...
            titleLabel.backgroundColor = [KxMenu sharedMenu].itemBgColor?:[UIColor clearColor];
            if (menuItem.action==nil) {
                titleLabel.textColor = [UIColor getColor:@"0x000000"];
            }
            if ([KxMenu sharedMenu].itemWidth>0) {
                titleLabel.width = [KxMenu sharedMenu].itemWidth;
                titleLabel.x=([KxMenu sharedMenu].defineWidth - [KxMenu sharedMenu].itemWidth)/2.0;
                titleLabel.textAlignment = NSTextAlignmentCenter;
            }
            if ([KxMenu sharedMenu].itemHeight>0) {
                titleLabel.height = [KxMenu sharedMenu].itemHeight;
                titleLabel.centerY = itemView.height/2.0;
            }
            if([KxMenu sharedMenu].curIndex!=-1){
                if([_menuItems indexOfObject:menuItem]==[KxMenu sharedMenu].curIndex){
                    titleLabel.textColor = [KxMenu sharedMenu].selectTextColor;
                    titleLabel.backgroundColor = [KxMenu sharedMenu].itemBgColor?:[UIColor clearColor];
                    if([KxMenu sharedMenu].itemCornerRadius){
                        [itemView addSubview:titleLabel];
                        [titleLabel cornerRadiusWithFloat:[KxMenu sharedMenu].itemCornerRadius];
                        [titleLabel addShadowWithShadowOffsetByAddView:CGSizeMake(0,0)];
                    }else{
                        [titleLabel cornerRadiusWithFloat:0];
                    }
                }else{
                    titleLabel.backgroundColor = [UIColor clearColor];
                    [titleLabel removeShadowView];
                }
            }
        ...
        if (itemNum < _menuItems.count - 1) {
            ...
            if ([KxMenu sharedMenu].lineColor) {
                lineView.backgroundColor=[KxMenu sharedMenu].lineColor;
            }
...
{
    self.x-=[KxMenu sharedMenu].offsetX;
...
- (void)drawBackground:(CGRect)frame inContext:(CGContextRef) context
{
    ...
    if (_arrowDirection == KxMenuViewArrowDirectionUp) {
        ...
        const CGFloat arrowXM = _arrowPosition+[KxMenu sharedMenu].offsetX;;
    ...
    UIBezierPath *borderPath = [UIBezierPath bezierPathWithRoundedRect:bodyFrame cornerRadius:[KxMenu sharedMenu].cornerRadius];
