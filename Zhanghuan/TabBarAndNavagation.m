...
+ (UIBarButtonItem *)setLeftBarButtonFrame:(CGRect)frame normolImageName:(NSString *)normalmageName pressImageName:(NSString *)pressmageName target:(UIViewController *)target action:(SEL)action{
    ...
    [btn setImage:[UIImage imageNamed:normalmageName] forState:UIControlStateNormal];
    [btn setImage:[UIImage imageNamed:pressmageName] forState:UIControlStateHighlighted];
...
+ (void)popViewController:(NSString *)viewController toTarget:(UIViewController *)target backShowTabBar:(BOOL)backShow{
    ...
    if (controllerTarget) {
        controllerTarget.hidesBottomBarWhenPushed=!backShow;
        [target.navigationController popToViewController:controllerTarget animated:YES]; //跳转
    }
}
