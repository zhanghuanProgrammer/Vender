...
+ (id)settingCellWithTableView:(UITableView*)tableView item:(RTSettingItem *)item{
    ...
    if (item.subTitle && item.subTitle.length > 0) {
        ...
        if (cell == nil) {
            cell = [[RTSettingCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:ID_Value1 item:item];
            
        }
    }else if (item.detail && item.detail.length > 0) {
        cell = [tableView dequeueReusableCellWithIdentifier:ID_Subtitle];
        if (cell == nil) {
            cell = [[RTSettingCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:ID_Subtitle item:item];
        }
    }else{
        cell = [tableView dequeueReusableCellWithIdentifier:ID_Default];
        if (cell == nil) {
            cell = [[RTSettingCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID_Default item:item];
        }
    }
    cell.item = item;
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString*)reuseIdentifier item:(RTSettingItem *)item{
    self = [super initWithStyle:style reuseIdentifier:reuseIdentifier];
    if (self) {
        if (item.type == ZFSettingItemTypeArrow && item.customArrowView) {
            [self.contentView addSubview:item.customArrowView];
        }
    }
    return self;
}
...
- (void)setItem:(RTSettingItem*)item{
    ...
    if (item.isEdit) {
        ...
        if (item.type == ZFSettingItemTypeArrow) {
            ...
            // 什么也没有，清空右边显示的view
            self.accessoryView = nil;
            if (item.customArrowView) {
                self.accessoryView = item.customArrowView;
            }
            // 用默认的选中样式
            self.selectionStyle = UITableViewCellSelectionStyleBlue;
        }
